...
public interface LambdStreamExc {

  /**
   * Create a string stream from array
   * note: arbitrary number of values will be stored in array
   * @param settings
   * @return
   *
   */
Stream<String>  createStrStream(String... strings);
  ...
  /**
   * Filter string that contains the pattern
   * for e.g filter(stringstream,"a") will return another stream with no a
   * @paran stringstream
   * @param pattern
   * @return
   */
